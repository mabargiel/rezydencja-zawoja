import type { Messages } from '../types'

export const de = {
  meta: {
    siteName: 'Rezydencja Zawoja',
    home: {
      title: 'Rezydencja Zawoja · Ihr eigenes Haus in den Bergen',
      description:
        'Eine luxuriöse Residenz zur Alleinnutzung in Zawoja, am Fuß der Babia Góra. Ein Blockhaus mit Wellnessbereich und Blick auf die Beskiden für bis zu 10 Gäste.',
    },
    interiors: {
      title: 'Innenräume',
      description:
        'Ein Blockhaus mit antiken Möbeln und fünf Schlafzimmern. Entdecken Sie die Innenräume der Rezydencja Zawoja Raum für Raum.',
    },
    surroundings: {
      title: 'Umgebung',
      description:
        'Babia Góra, Skipisten, Wanderwege und Wasserfälle. Was Sie in Zawoja und Umgebung erleben können.',
    },
    gallery: {
      title: 'Galerie',
      description:
        'Fotos der Rezydencja Zawoja: Innenräume, Wellnessbereich, Terrasse, Garten und Blick auf die Babia Góra.',
    },
    contact: {
      title: 'Kontakt',
      description:
        'Fragen Sie nach freien Terminen in der Rezydencja Zawoja. Wir antworten noch am selben Tag.',
    },
  },
  nav: {
    label: 'Hauptnavigation',
    homeLink: 'Rezydencja Zawoja, Startseite',
    home: 'Start',
    interiors: 'Innenräume',
    surroundings: 'Umgebung',
    gallery: 'Galerie',
    pricing: 'Preise',
    contact: 'Kontakt',
    book: 'Buchen',
    openMenu: 'Menü öffnen',
    closeMenu: 'Menü schließen',
    language: 'Sprache',
  },
  footer: {
    label: 'Fußzeile',
    navigation: 'Navigation in der Fußzeile',
    address: 'Zawoja, Beskiden, am Fuß der Babia Góra',
    copyright: '© {{year}} Rezydencja Zawoja. Alle Rechte vorbehalten.',
  },
  pages: {
    home: {
      eyebrow: 'Beskiden · am Fuß der Babia Góra',
      title: 'Ein Haus in den Bergen,\nnur für Sie',
      intro:
        'Eine luxuriöse Residenz zur Alleinnutzung. Stille, viel Raum und der Blick auf die Babia Góra aus jedem Fenster.',
    },
    interiors: {
      eyebrow: 'Innenräume',
      title: 'Ein Haus mit Seele,\nRaum für Raum',
      intro:
        'Ein Blockhaus, in dem antike Möbel auf gehobene Ausstattung treffen. Jeder Raum hat seinen eigenen Charakter.',
    },
    surroundings: {
      eyebrow: 'Umgebung',
      title: 'Am Fuß der\nBabia Góra',
      intro:
        'Zawoja ist das längste Dorf Polens. Es erstreckt sich über 18 km durch die Saybuscher Beskiden, direkt unter der majestätischen Königin der Beskiden.',
    },
    gallery: {
      eyebrow: 'Galerie',
      title: 'Ein Blick in\njeden Winkel',
      intro:
        'Räume voller Charakter, ein Ort zum Entspannen und Aussichten, die lange in Erinnerung bleiben. Entdecken Sie die Rezydencja Zawoja von innen und außen.',
    },
    contact: {
      eyebrow: 'Kontakt',
      title: 'Willkommen in\nder Residenz',
      intro:
        'Bei Fragen oder für eine Buchung sind wir gern für Sie da. Rufen Sie an oder nutzen Sie das Formular, wir bestätigen die Verfügbarkeit noch am selben Tag.',
    },
    notFound: {
      eyebrow: 'Fehler 404',
      title: 'Diese Seite\ngibt es nicht',
      intro: 'Die gesuchte Seite existiert nicht oder wurde verschoben.',
      back: 'Zur Startseite',
    },
  },
} satisfies Messages
